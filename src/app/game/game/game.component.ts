...
export class GameComponent implements OnInit {
  ...
  ngOnInit(): void {
        ...
        map(
          ...
          .filter(
            x => [
              ...
              .map(value => console.log([
                    data.requestTwo
                    .filter(
                      x => x.TeamID === value.TeamID
                    ).map(x => [
                      x.TeamID,
                      x.WikipediaLogoUrl,
                      value.Wins === 1,
    ...
    // this.nbaService.requestDataFromMultipleSources(currentDateTime)
    // .subscribe(res => console.log(res));

    // this.nbaService.getAllTeams()
    // .pipe(map(data => data.filter(teams => teams.Key === "NO")))
